...
%Now by the uniform continuity of $\phi$, for any $\epsilon>0$ we have $|\phi(x)-\phi(y)|< \epsilon$ for $|x-y|<\delta$ for some $\delta>0$.
%
%\begin{eqnarray*}
%|\phi(x) f_{b,n}(x)-\phi(y) f_{b,n}(y)| & = & |\phi(x) f_{b,n}(x)-\phi(y) f_{b,n}(x) + \phi(y) f_{b,n}(x) -\phi(y) f_{b,n}(y)|\\
%& \leq & |\phi(x) f_{b,n}(x)-\phi(y) f_{b,n}(x)| + |\phi(y) f_{b,n}(x) -\phi(y) f_{b,n}(y)|\\
%& = & |\phi(x)-\phi(y)||f_{b,n}(x)| + |\phi(y) || f_{b,n}(x) -f_{b,n}(y)|\\
%& \leq & \epsilon \frac{1}{b} + M \frac{1}{b^2} |x-y|\\
%& \leq & \epsilon \frac{1}{b} + M \frac{1}{b^2} \delta.
%\end{eqnarray*}
%
%Selecting $0<\epsilon<\frac{b\eps}{4K}$, choose $N$ with $2K/N \leq min(\delta,\frac{b^2\eps}{4MK})$. Then partitioning the interval $[-K,K]$ into $N$ subintervals of equal length with endpoints $-K = a_0 < a_1 < \cdots < a_N = K$, we can estimate the integral by the Riemann sum $\Delta_n$:
%
%\begin{eqnarray*}
%|\int_{\R} \phi(x) f_{b,n}(x) dx - \Delta_n | & =  & |\int_{\R} \phi(x) f_{b,n}(x) dx - \frac{2K}{N} \sum_{i=1}^N \phi(a_i)f_{b,n}(a_i) |\\
%& = & |\sum_{i=1}^N(\int_{a_{i-1}}^{a_i} \phi(x) f_{b,n}(x) dx - \frac{2K}{N} \phi(a_i)f_{b,n}(a_i)) |\\
%& \leq & \sum_{i=1}^N|\int_{a_{i-1}}^{a_i} \phi(x) f_{b,n}(x) dx - \frac{2K}{N} \phi(a_i)f_{b,n}(a_i)|\\
%& = & \sum_{i=1}^N|\int_{a_{i-1}}^{a_i} \phi(x) f_{b,n}(x) - \phi(a_i)f_{b,n}(a_i) dx|\\
%& \leq & \sum_{i=1}^N\int_{a_{i-1}}^{a_i} |\phi(x) f_{b,n}(x) - \phi(a_i)f_{b,n}(a_i)| dx\\
%& \leq & \sum_{i=1}^N\int_{a_{i-1}}^{a_i} \frac{b\eps}{4K} \frac{1}{b} + M \frac{1}{b^2} \frac{b^2\eps}{4MK} dx\\
%& = & \sum_{i=1}^N \frac{2K}{N} \frac{b\eps}{4K} \frac{1}{b} + M \frac{1}{b^2} \frac{b^2\eps}{4MK}\\
%& = & 2K (\frac{\eps}{4K} + \frac{\eps}{4K})\\
%& = & \eps.
%\end{eqnarray*}
%
%The exact same inequality works if we replace $f_{b,n}$ by $f_b$.
%
%Now finally, every sequence $f_{b,n}(a_i)$ converges almost surely to $f_{b}(a_i)$, because we assumed that the Stieltjes transforms converge pointwise almost surely. This means that we can find almost surely an index $n$
%
%Partitions Riemann sums almost sure approximation by imaginary part uniform blah blah blah.
